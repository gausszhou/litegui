...
function createMenu() {
  ...
  mainmenu.add("Debug/Modal", {
    callback: function () {
      var dialog = new LiteGUI.Dialog("blarg", {
        width: 300,
        height: 100,
        close: true,
        content: "This is an example of modal dialog"
      });
      dialog.makeModal();
...
function createSidePanel() {
  ...
  LiteGUI.bind(panel, "closed", function () {
    mainarea.merge();
  });
...
function updateSidePanel(root) {
  ...
  tabs_widget
    .getTabContent("Info")
    .appendChild(LiteGUI.createElement("strong", null, "Example of code inside tab container"));
  ...
  var mytree = {
    ...
    children: [
      ...
      {
        id: "Child2",
        children: [{ id: "SubChild1" }, { id: "SubChild2" }, { id: "SubChild3" }, { id: "SubChild4" }]
      },
      { id: "Child3" }
    ]
  };
  ...
  widgets.addStringButton("String Button", "foo", {
    callback: function (v) {
      console.log("Button: " + v);
    }
  });
  widgets.addTextarea(null, "a really long silly text", { height: 100 });
  widgets.addCombo("Combo", "javi", {
    values: ["foo", "faa", "super largo texto que no cabe entero", "javi", "nada"],
    callback: function (name) {
      console.log("Combo selected: " + name);
    }
  });
  widgets.addComboButtons("Combobtns", "javi", {
    values: ["foo", "faa", "javi", "nada"],
    callback: function (name) {
      console.log("Combo button selected: " + name);
    }
  });
  widgets.addTags("Tags", "pop", {
    values: ["rap", "blues", "pop", "jazz"],
  ...
  widgets.addPad("Pad", [0.5, 0.5], function (v) {
    console.log(v);
  });
...
function createComplexListDialog() {
  ...
  for (var i = 0; i < 10; ++i) list.addItem({}, "Example", Math.random() > 0.5, true);
  list.addTitle("Example of title");
  for (var i = 0; i < 10; ++i) list.addItem({}, "More items", Math.random() > 0.5, true);
...
function createWidgetsDialog() {
  ...
  widgets.addButton("button", "Update", {
    callback: function () {
      updateSidePanel();
    }
  });
