...
Menu.prototype.updateMenu = function () {
	...
	for (var i in this.menu) {
		...
		element.addEventListener("click", function (e) {
			...
			if (item.data && item.data.callback && typeof item.data.callback == "function") item.data.callback(item.data);
...
Menu.prototype.hidePanels = function () {
	...
	for (var i in this.panels) LiteGUI.remove(this.panels[i]);
	this.panels = [];
};
...
Menu.prototype.showMenu = function (menu, e, root, is_submenu) {
	...
	var sorted_entries = [];
	for (var i in menu.children) sorted_entries.push(menu.children[i]);
