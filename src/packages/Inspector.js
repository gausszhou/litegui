...
function Inspector(options) {
	...
	if (options.width) this.root.style.width = LiteGUI.sizeToCSS(options.width);
	if (options.height) {
...
Inspector.prototype.getValues = function () {
	var r = {};
	for (var i in this.widgets_by_name) r[i] = this.widgets_by_name[i].getValue();
	return r;
};

Inspector.prototype.setValues = function (v) {
	for (var i in v) if (this.widgets_by_name[i]) this.widgets_by_name[i].setValue(v[i]);
};
...
Inspector.prototype.appendTo = function (parent, at_front) {
	...
	if (!parent) return;
	if (at_front) parent.insertBefore(this.root, parent.firstChild);
...
Inspector.prototype.clear = function () {
	...
	while (this.root.hasChildNodes()) this.root.removeChild(this.root.lastChild);
...
Inspector.prototype.append = function (widget, options) {
	...
	if (options.replace) options.replace.parentNode.replaceChild(widget, options.replace);
	else {
...
Inspector.prototype.pushContainer = function (container) {
	if (!this._current_container_stack) this._current_container_stack = [container];
	else {
...
Inspector.prototype.isContainerInStack = function (container) {
	if (!this._current_container_stack) return false;
	if (this._current_container_stack.indexOf(container) != -1) return true;
	return false;
};
...
Inspector.prototype.popContainer = function (container) {
	...
	if (this._current_container_stack && this._current_container_stack.length) {
		if (container) {
			...
			while (aux && aux != container) aux = this._current_container_stack.pop();
		} else {
	...
	} else this._current_container = null;
};
...
Inspector.prototype.getWidget = function (name) {
	if (name !== null && name.constructor === Number) return this.widgets[name];
...
Inspector.prototype.inspectInstance = function (instance, properties, properties_info_example, properties_to_skip) {
	...
	if (!properties) {
		if (instance.getProperties) properties = instance.getProperties();
		else properties = this.collectProperties(instance);
	} else {
	}

	var classObject = instance.constructor;
	if (!properties_info_example && classObject.properties) properties_info_example = classObject.properties;
	...
	if (instance.getInspectorProperties) properties_info = instance.getInspectorProperties();
	else {
			...
			if (classObject["@" + i]) {
				...
				switch (v.constructor) {
					...
					case Boolean:
						...
						break;
					default:
						if (v && (v.constructor === Array || v.constructor.BYTES_PER_ELEMENT)) {
							//Array or typed_array
	...
	if (properties_to_skip) for (var i in properties_to_skip) delete properties_info[properties_to_skip[i]];
	...
	if (classObject.properties_order) {
		...
		for (var i in classObject.properties_order) {
			...
			if (properties_info[name]) sorted_properties[name] = properties_info[name];
			else console.warn("property not found in instance:", name);
		}
		for (var i in properties_info) { //add the missing ones at the end (should this be optional?)
			if (!sorted_properties[i]) sorted_properties[i] = properties_info[i];
		}
	...
	function inner_clone(original, target) {
		target = target || {};
		for (var j in original) target[j] = original[j];
		return target;
	}
};
...
Inspector.prototype.collectProperties = function (instance) {
	...
	for (var i in instance) {
		if (i[0] == "_" || i[0] == "@" || i.substr(0, 6) == "jQuery")
			//skip vars with _ (they are private)
			continue;

		var v = instance[i];
		if (v && v.constructor == Function && !instance.constructor["@" + i]) continue;
		properties[i] = v;
	}
	return properties;
};
...
Inspector.prototype.showProperties = function (instance, properties_info) {
	...
	for (var i in properties_info) {
		...
		if (options.constructor === String)
			//it allows to just specify the type
			options = { type: options };
		if (options.name) varname = options.name;
		if (!options.callback) {
			//generate default callback to modify data
			var o = { instance: instance, name: varname, options: options };
			if (options.type == "function") {
			} else options.callback = Inspector.assignValue.bind(o);
		}
		if (!options.callback_update) {
			//generate default refresh
		...
		if (this.on_addProperty) this.on_addProperty(type, instance, varname, instance[varname], options);
	...
	if (instance.constructor.onShowProperties) instance.constructor.onShowProperties(instance, this);
};
...
Inspector.assignValue = function (value) {
	...
	if (current_value == null || value == null || this.options.type == "enum") instance[this.name] = value;
	else if (typeof current_value == "number") instance[this.name] = parseFloat(value);
	else if (typeof current_value == "string") instance[this.name] = value;
	else if (
		value &&
		value.length &&
		current_value &&
		...
		(!Object.getOwnPropertyDescriptor(instance.__proto__, this.name) ||
			!Object.getOwnPropertyDescriptor(instance.__proto__, this.name).set)
	) {
		for (var i = 0; i < value.length; ++i) current_value[i] = value[i];
...
Inspector.prototype.createWidget = function (name, content, options) {
	...
	if (width) {
		...
		if (!element.style.width) element.style.width = "calc(" + LiteGUI.sizeToCSS(width) + ")";
		element.style.minWidth = "auto";
	}
	var height = options.height || this.height;
	if (height) {
		element.style.height = LiteGUI.sizeToCSS(height);
		if (!element.style.height) element.style.height = "calc(" + LiteGUI.sizeToCSS(height) + ")";
		element.style.minHeight = "auto";
	}

	//store widgets
	this.widgets.push(element);
	if (options.widget_name || name) this.widgets_by_name[options.widget_name || name] = element;
	...
	if (name !== undefined && name !== null && (this.name_width || options.name_width) && !this.one_line) {
		...
		namewidth =
			"style='width: calc(" + w + " - 0px); width: -webkit-calc(" + w + " - 0px); width: -moz-calc(" + w + " - 0px); '"; //hack
		contentwidth =
	...
	if (name === null || name === undefined) content_class += " full";
	else if (name === "")
		//three equals because 0 == ""
	...
	if (content.constructor === String || content.constructor === Number || content.constructor === Boolean)
		element.innerHTML =
			code + "<span class='info_content " + content_class + "' " + contentwidth + ">" + content + "</span>";
	else {
...
Inspector.onWidgetChange = function (element, name, value, options, expand_value, event) {
	...
	var r = undefined;
	if (options.callback) {
		if (expand_value) r = options.callback.apply(element, value);
		else r = options.callback.call(element, value, event);
	}

	if (!options.skip_wchange) {
		if (section) LiteGUI.trigger(section, "wchange", value, element);
...
Inspector.prototype.add = function (type, name, value, options) {
	if (!type) throw "Inspector: no type specified";
	...
	if (func.constructor === String) func = LiteGUI.Inspector.prototype[func];
	if (!func) return;
	if (func.constructor !== Function) return;

	if (options && options.constructor === Function) options = { callback: options };
...
Inspector.prototype.applyOptions = function (element, options) {
	...
	if (options.width) element.style.width = LiteGUI.sizeToCSS(options.width);
	if (options.height) element.style.height = LiteGUI.sizeToCSS(options.height);
};
...
Inspector.prototype.addDefault = function (name, value, options) {
	if (value === null || value === undefined)
		//can we guess it from the current value?
		return null;

	if (value.constructor === Boolean) return this.addCheckbox(name, value, options);
	else if (value.constructor === String) return this.addString(name, value, options);
	else if (value.constructor === Number) return this.addNumber(name, value, options);
	else if (value.length == 4) return this.addVector4(name, value, options);
	else if (value.length == 3) return this.addVector3(name, value, options);
	else if (value.length == 2) return this.addVector2(name, value, options);
	return null;
};
...
Inspector.prototype.addStringButton = function (name, value, options) {
	...
	var element = this.createWidget(
		name,
		"<span class='inputfield button'><input type='text' tabIndex='" +
			this.tab_index +
			"' class='text string' value='' " +
			(options.disabled ? "disabled" : "") +
			"/></span><button class='micro'>" +
			(options.button || "...") +
			"</button>",
		options
	);
	...
	button.addEventListener("click", function (e) {
		if (options.callback) options.callback.call(element, input.value, e);
	});
...
Inspector.prototype.addTextarea = function (name, value, options) {
	...
	if (options.callback_keydown) textarea.addEventListener("keydown", options.callback_keydown);

	if (options.height) textarea.style.height = "calc( " + LiteGUI.sizeToCSS(options.height) + " - 5px )";
	//textarea.style.height = LiteGUI.sizeToCSS( options.height );
	this.append(element, options);
	element.setValue = function (v, skip_event) {
		if (v === undefined) return;
		if (v == textarea.value) return;
		value = v;
		textarea.value = v;
...
Inspector.prototype.addNumber = function (name, value, options) {
	...
	function inner_before_change(e) {
		if (this.callback_before) this.callback_before.call(element);
	}
	...
	input.addEventListener("change", function (e) {
		...
		if (options.callback) {
			...
			if (typeof ret == "number") this.value = ret;
		}
	...
	element.setValue = function (v, skip_event) {
		if (v === undefined) return;
		v = parseFloat(v);
		if (options.precision) v = v.toFixed(options.precision);
		v += options.units || "";
...
Inspector.prototype.addVector2 = function (name, value, options) {
	...
	for (var i = 0; i < inputs.length; ++i)
		inputs[i].addEventListener("change", function (e) {
			...
			var r = [];
			var elems = inputs;
			for (var i = 0; i < elems.length; i++) r.push(parseFloat(elems[i].value));
			...
			if (options.callback) {
				...
				if (typeof new_val == "object" && new_val.length >= 2) {
					for (var i = 0; i < elems.length; i++) elems[i].value = new_val[i];
					r = new_val;
				}
			}
	...
	element.setValue = function (v, skip_event) {
		...
		if (dragger2.getValue() != v[1]) dragger2.setValue(v[1], skip_event); //last one triggers the event
	};
	element.setRange = function (min, max) {
...
Inspector.prototype.addVector4 = function (name, value, options) {
	...
	element.setValue = function (v, skip_event) {
		if (!v) return;
		for (var i = 0; i < draggers.length; i++) draggers[i].setValue(v[i], skip_event);
	};
	element.setRange = function (min, max) {
		...
		if (e.type == "mousedown") {
			...
			if (options.callback) {
				...
				if (new_val && new_val.length >= 2) {
					for (var i = 0; i < elems.length; i++) element.setValue(new_val);
				}
			}
...
Inspector.prototype.addInfo = function (name, value, options) {
	...
	var element = null;
	if (name != null) element = this.createWidget(name, value, options);
	else {
		...
		} else element.innerHTML = "<span class='winfo'>" + value + "</span>";
	}
	...
	if (!content) content = element.querySelector(".winfo");
...
Inspector.prototype.addSlider = function (name, value, options) {
	...
	if (options.step === undefined) options.step = 0.01;
...
Inspector.prototype.addCheckbox = function (name, value, options) {
	...
	var element = this.createWidget(
		name,
		"<span class='inputfield'><span tabIndex='" +
			this.tab_index +
			"' class='fixed flag checkbox " +
			(value ? "on" : "off") +
			"'>" +
			label +
			"</span></span>",
	...
	checkbox.addEventListener("keypress", function (e) {
		if (e.keyCode == 32) LiteGUI.trigger(this, "click");
	});

	element.addEventListener("click", function () {
		value = !value;
		element.querySelector("span.flag").innerHTML = value ? label_on : label_off;
		if (value) checkbox.classList.add("on");
	...
	element.setValue = function (v, skip_event) {
		if (v === undefined) return;
		value = v;
		if (that.values[name] != v && !skip_event) LiteGUI.trigger(checkbox, "click");
	};
...
Inspector.prototype.addFlags = function (flags, force_flags, options) {
	var f = {};
	for (var i in flags) f[i] = flags[i];
	if (force_flags) for (var i in force_flags) if (typeof f[i] == "undefined") f[i] = force_flags[i] ? true : false;

	for (var i in f) {
		var flag_options = {};
		for (var j in options) flag_options[j] = options[j];
...
Inspector.prototype.addCombo = function (name, value, options) {
	...
	var element = this.createWidget(
		name,
		"<span class='inputfield full inputcombo " + (options.disabled ? "disabled" : "") + "'></span>",
		options
	);
	element.options = options;

	var values = options.values || [];
	if (values.constructor === Function) values = options.values();
  ...
  */
	...
	element.setValue = function (v, skip_event) {
		...
		var index = -1;
		if (values.constructor === Array) index = values.indexOf(v);
		else {
	...
	function setValues(v, selected) {
		...
		for (var i in values) {
			...
			var item_index = values.constructor === Array ? index : i;
			var item_title = values.constructor === Array ? item_value : i;
			if (item_value && item_value.title) item_title = item_value.title;
			code +=
				"<option value='" +
				item_index +
				"' " +
				(item_value == value ? " selected" : "") +
				" data-index='" +
				item_index +
				"'>" +
				item_title +
...
Inspector.prototype.addComboButtons = function (name, value, options) {
	...
	if (options.values)
		for (var i in options.values)
			code +=
				"<button class='wcombobutton " +
				(value == options.values[i] ? "selected" : "") +
				"' data-name='options.values[i]'>" +
				options.values[i] +
				"</button>";
...
Inspector.prototype.addTags = function (name, value, options) {
	...
	var code = "<select>";
	if (options.values) for (var i in options.values) code += "<option>" + options.values[i] + "</option>";
	...
	//add default tags
	for (var i in options.value) inner_addtag(options.value[i]);
	...
	function inner_addtag(tagname) {
		if (element.tags[tagname]) return; //repeated tags no
...
Inspector.prototype.addList = function (name, values, options) {
	...
	function inner_key(e) {
		...
		if (e.keyCode == 13) {
			...
			if (selected.scrollIntoViewIfNeeded) selected.scrollIntoViewIfNeeded({ block: "end", behavior: "smooth" });
	...
	function inner_item_click(e) {
		if (options.multiselection) this.classList.toggle("selected");
		else {
			//batch action, jquery...
	...
	function insert_item(value, selected, index) {
		...
		if (options.callback_dblclick) li_element.addEventListener("dblclick", inner_item_dblclick);
		return li_element;
	}
	...
	element.removeItem = function (name) {
		...
		for (var i = 0; i < items.length; i++) {
			if (items[i].dataset["name"] == name) LiteGUI.remove(items[i]);
		}
	};
	...
	element.getSelected = function () {
		...
		for (var i = 0; i < selected.length; ++i) r.push(selected[i].dataset["name"]);
		return r;
	};
	...
	element.selectIndex = function (num, add_to_selection) {
		...
		for (var i = 0; i < items.length; ++i) {
			var item = items[i];
			if (i == num) item.classList.add("selected");
			else if (!add_to_selection) item.classList.remove("selected");
		}
		return items[num];
	};
	...
	element.filter = function (callback, case_sensitive) {
		...
		for (var i = 0; i < items.length; ++i) {
			...
			if (use_string && value != null && value.constructor !== String) value = item.innerHTML;

			if (!callback(value, item, item.classList.contains("selected"))) item.style.display = "none";
	...
	element.selectByFilter = function (callback) {
		...
		for (var i = 0; i < items.length; ++i) {
			...
			if (r === true) item.classList.add("selected");
			else if (r === false) item.classList.remove("selected");
		}
	};
...
Inspector.prototype.addButton = function (name, value, options) {
	...
	if (options.micro) button_classname += " micro";
	...
	element.wclick = function (callback) {
		if (!options.disabled) LiteGUI.bind(this, "wclick", callback);
	};
...
Inspector.prototype.addButtons = function (name, value, options) {
	...
	if (value && typeof value == "object") {
		for (var i in value) {
			var title = "";
			if (options.title && options.title.constructor === Array) title = options.title[i] || "";
			code +=
				"<button class='litebutton' title='" +
				title +
				"' tabIndex='" +
				this.tab_index +
				"' style='" +
...
Inspector.prototype.addIcon = function (name, value, options) {
	...
	var x = options.x || 0;
	if (options.index) x = options.index * -width;
	...
	element.setValue = function (v, skip_event) {
		...
		var y = value ? height : 0;
		icon.style.backgroundPosition = x + "px " + y + "px";
		if (!skip_event) Inspector.onWidgetChange.call(that, element, name, value, options);
	};
	element.getValue = function () {
		return value;
	};
...
Inspector.prototype.addColor = function (name, value, options) {
    ...
    */
		...
		myColor.onImmediateChange = function () {
			var v = [
				myColor.rgb[0] * myColor.rgb_intensity,
				myColor.rgb[1] * myColor.rgb_intensity,
			...
			that.values[name] = v;
			if (options.callback) options.callback.call(element, v.concat(), "#" + myColor.toString(), myColor);
		...
		dragger.input.addEventListener("change", function (e) {
			...
			if (myColor.onImmediateChange) myColor.onImmediateChange();
		});
...
Inspector.prototype.addFile = function (name, value, options) {
	...
	var element = this.createWidget(
		name,
		"<span class='inputfield full whidden' style='width: calc(100% - 26px)'><span class='filename'></span></span><button class='litebutton' style='width:20px; margin-left: 2px;'>...</button><input type='file' size='100' class='file' value='" +
			value +
			"'/>",
		options
	);
	...
	input.addEventListener("change", function (e) {
		...
		if (options.generate_url) file.url = URL.createObjectURL(e.target.files[0]);
		...
		if (options.read_file) {
			...
			if (options.read_file == "binary") reader.readAsArrayBuffer(file);
			else if (options.read_file == "data_url") reader.readAsDataURL(file);
...
Inspector.prototype.addDataTree = function (name, value, options) {
	...
	function inner_recursive(root_node, value) {
		for (var i in value) {
			...
			} else e.innerHTML = "<span class='itemname'>" + i + "</span><span class='itemvalue'>" + value[i] + "</span>";
...
Inspector.prototype.addArray = function (name, value, options) {
	...
	this.addButtons(null, ["+", "-"], {
		width: 60,
		callback: function (v) {
			if (v == "+") value.length = value.length + 1;
			else if (value.length > 0) value.length = value.length - 1;
	...
	function refresh() {
		...
		for (var i = 0; i < size; ++i) {
			...
			var item_options = {
				...
				callback: assign.bind({ value: this.value, index: i })
			};
			if (options.data_options) for (var j in options.data_options) item_options[j] = options.data_options[j];
	...
	function assign(v) {
		...
		if (options.callback) options.callback.call(container, this.value, this.index);
		//todo: trigger change
	}
...
Inspector.prototype.startContainer = function (name, options) {
	...
	if (options.widgets_per_row) this.widgets_per_row = options.widgets_per_row;
...
Inspector.prototype.addSection = function (name, options) {
	...
	if (options.className) element.className += " " + options.className;
	if (options.collapsed) element.className += " collapsed";
	...
	if (name)
		code +=
			"<div class='wsectiontitle'>" +
			(options.no_collapse ? "" : "<span class='switch-section-button'></span>") +
			name +
			"</div>";
	...
	if (name)
		element.sectiontitle.addEventListener("click", function (e) {
			...
			if (options.callback) options.callback.call(element, !element.classList.contains("collapsed"));
		});

	if (options.collapsed) element.querySelector(".wsectioncontent").style.display = "none";
...
Inspector.prototype.getCurrentSection = function () {
	for (var i = this._current_container_stack.length - 1; i >= 0; --i) {
		...
		if (container.classList.contains("wsectioncontent")) return container.parentNode;
	}
	return null;
};
...
Inspector.prototype.beginGroup = function (name, options) {
	...
	name = name || "";
	element.innerHTML =
		"<div class='wgroupheader " +
		(options.title ? "wtitle" : "") +
		"'><span class='switch-section-button'></span>" +
		name +
		"</div>";
	element.group = true;
...
Inspector.parseColor = function (color) {
	return (
		...
		color[0].toFixed(2) +
		"</span>,<span style='color: #AFA'>" +
		color[1].toFixed(2) +
		"</span>,<span style='color: #AAF'>" +
		color[2].toFixed(2) +
		"</span>"
	);
};
