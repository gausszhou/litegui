...
Area.prototype.getSection = function (num) {
	num = num || 0;
	if (this.sections.length > num) return this.sections[num];
	return null;
};
...
Area.prototype.onResize = function (e) {
	...
	if (
		e &&
		this._computed_size &&
		computed_size[0] == this._computed_size[0] &&
		computed_size[1] == this._computed_size[1]
	)
		return;
...
Area.prototype.sendResizeEvent = function (e) {
	...
	//send it to the children
		...
		for (var j = 0; j < this.root.childNodes.length; j++) {
			...
			if (element.litearea) element.litearea.onResize();
			else LiteGUI.trigger(element, "resize");
		}
	}
...
Area.prototype.split = function (direction, sizes, editable) {
	...
	function inner_mousemove(e) {
		if (direction == "horizontal") {
			if (last_pos[0] != e.pageX) that.moveSplit(last_pos[0] - e.pageX);
		} else if (direction == "vertical") {
			if (last_pos[1] != e.pageY) that.moveSplit(e.pageY - last_pos[1]);
		}
		...
		e.stopPropagation();
		e.preventDefault();
		if (that.options.immediateResize || that.options.inmediateResize)
...
Area.prototype.showSection = function (num) {
	...
	else size = section.root.style.height;
	...
	for (var i in this.sections) {
		...
		if (i == num) section.root.style.display = "inline-block";
		else {
			if (this.direction == "horizontal") section.root.style.width = "calc( 100% - " + size + " - 5px)";
			else section.root.style.height = "calc( 100% - " + size + " - 5px)";
		}
	}
...
Area.prototype.hideSection = function (num) {
	for (var i in this.sections) {
		...
		if (i == num) section.root.style.display = "none";
		else {
			if (this.direction == "horizontal") section.root.style.width = "100%";
			else section.root.style.height = "100%";
		}
	}
...
Area.prototype.moveSplit = function (delta) {
	...
	for (var i = 0; i < areas.length; ++i) LiteGUI.trigger(areas[i], "split_moved");
};
