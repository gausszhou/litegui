...
function Table(options) {
	...
	if (options.colums) this.setColumns(options.colums);
	...
	if (options.rows) this.setRows(options.data);
}
...
Table.prototype.addRow = function (row, skip_add) {
	...
	for (var j = 0; j < this.column_fields.length; ++j) {
		...
		//object
		else value = row[this.column_fields[j]];
...
Table.prototype.setColumns = function (columns) {
	...
	for (var i = 0; i < columns.length; ++i) {
		...
		if (c.constructor === String || c.constructor === Number) c = { name: String(c) };
