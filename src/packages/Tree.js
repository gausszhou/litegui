...
function Tree(data, options, legacy) {
	...
	if (!root_item) throw "Error in LiteGUI.Tree, createAndInsert returned null";
...
Tree.prototype.insertItem = function (data, parent_id, position, options) {
	if (!parent_id) {
		...
		if (root) parent_id = root.dataset["item_id"];
	}

	var element = this.createAndInsert(data, options, parent_id, position);

	//update parent collapse button
	if (parent_id) this._updateListBox(this._findElement(parent_id)); //no options here, this is the parent
...
Tree.prototype.createAndInsert = function (data, options, parent_id, element_index) {
	...
	if (parent_id) parent_element_index = this._findElementIndex(parent_id);
	else if (parent_id === undefined) parent_element_index = 0; //root
	...
	//insert
	if (parent_element_index == -1) this.root.appendChild(element);
	else this._insertInside(element, parent_element_index, element_index);

	//compute visibility according to parents
	if (parent && !this._isNodeChildrenVisible(parent_id)) element.classList.add("hidden");
	...
	if (options && options.selected) this.markAsSelected(element, true);
...
Tree.prototype._insertInside = function (element, parent_index, offset_index, level) {
	...
	if (!parent) throw "No parent node found, index: " + parent_index + ", nodes: " + this.root.childNodes.length;
	...
	if (indent) indent.style.paddingLeft = (child_level + this.indent_offset) * Tree.INDENT + "px"; //inner padding
	...
	for (var j = parent_index + 1; j < this.root.childNodes.length; ++j) {
		...
		if (!new_childNode.classList || !new_childNode.classList.contains("ltreeitem")) continue;
...
Tree.prototype._findElement = function (id) {
	if (!id || id.constructor !== String) throw "findElement param must be string with item id";
	for (var i = 0; i < this.root.childNodes.length; ++i) {
		var childNode = this.root.childNodes[i];
		if (!childNode.classList || !childNode.classList.contains("ltreeitem")) continue;
		if (childNode.classList.contains("ltreeitem-" + id)) return childNode;
	}
...
Tree.prototype._findElementIndex = function (id) {
	for (var i = 0; i < this.root.childNodes.length; ++i) {
		...
		if (typeof id === "string") {
			if (childNode.dataset["item_id"] === id) return i;
		} else if (childNode === id) return i;
	}
...
Tree.prototype._findElementLastChildIndex = function (start_index) {
	...
	for (var i = start_index + 1; i < this.root.childNodes.length; ++i) {
		...
		if (current_level == level) return i;
	}
...
Tree.prototype.createTreeItem = function (data, options, level) {
	...
	if (data.dataset) for (var i in data.dataset) root.dataset[i] = data.dataset[i];
	...
	if (data.className) title_element.className += " " + data.className;
	...
	if (data.visible === false) root.style.display = "none";
	...
	row.addEventListener("contextmenu", function (e) {
		...
		if (that.onItemContextMenu) return that.onItemContextMenu(e, { item: item, data: item.data });
	...
	function onNodeSelected(e) {
		...
		if (e.ctrlKey && that.options.allow_multiselection) {
			...
			var r = false;
			if (data.callback) r = data.callback.call(that, node);

			if (!r && that.onItemAddToSelection) that.onItemAddToSelection(node.data, node);
		}
		if (e.shiftKey && that.options.allow_multiselection) {
			...
			var items =
				current_index > last_index
					? nodeList.slice(last_index, current_index)
					: nodeList.slice(current_index, last_index);
			...
			if (!r && that.onItemSelected) that.onItemSelected(node.data, node);
	...
	if (this.options.allow_drag) {
		...
		draggable_element.addEventListener("dragstart", function (ev) {
			...
			if (drag_data) {
				for (var i in drag_data) ev.dataTransfer.setData(i, drag_data[i]);
			}
		});
	}
	...
	draggable_element.addEventListener("dragleave", function (ev) {
		...
		//console.log(data.id);
		count--;
		if (count == 0) title_element.classList.remove("dragover");
	...
	draggable_element.addEventListener("drop", function (ev) {
		...
		if (!item_id) {
			...
			if (that.onDropItem) that.onDropItem(ev, this.parentNode.data);
			return;
		}
		...
		//try
		{
			...
			if (
				!that.onMoveItem ||
				(that.onMoveItem && that.onMoveItem(that.getItem(item_id), that.getItem(parent_id)) != false)
			) {
...
Tree.prototype.filterByName = function (name) {
	for (var i = 0; i < this.root.childNodes.length; ++i) {
		...
		if (!name || str.indexOf(name.toLowerCase()) != -1) {
			if (childNode.data && childNode.data.visible !== false) childNode.classList.remove("filtered");
			var indent = childNode.querySelector(".indentblock");
			if (indent) {
				if (name) indent.style.paddingLeft = 0;
				else
					indent.style.paddingLeft = paddingLeft =
						(parseInt(childNode.dataset["level"]) + this.indent_offset) * Tree.INDENT + "px";
			}
		} else {
		...
		if (callback_to_filter(childNode.data, content, param)) {
			...
			if (indent) {
				if (name) indent.style.paddingLeft = 0;
					...
					indent.style.paddingLeft = paddingLeft =
						(parseInt(childNode.dataset["level"]) + this.indent_offset) * LiteGUI.Tree.INDENT + "px";
			}
		} else {
			childNode.classList.add("filtered");
		}
...
Tree.prototype.getItem = function (id) {
	...
	for (var i = 0; i < this.root.childNodes.length; ++i) {
		...
		if (childNode.dataset["item_id"] === id) return childNode;
	}
...
Tree.prototype.isInsideArea = function (id) {
	...
	if (this.root.scrollTop < y && y < this.root.scrollTop + h) return true;
	return false;
};
...
Tree.prototype.scrollToItem = function (id) {
	...
	if (rect.width * 0.75 < x) container.scrollLeft = x;
...
Tree.prototype.getChildren = function (id, only_direct) {
	if (id && id.constructor !== String && id.dataset) id = id.dataset["item_id"];
...
Tree.prototype.moveItem = function (id, parent_id) {
	...
	if (children) {
		...
		//remove all children
		for (var i = 0; i < children.length; i++) children[i].parentNode.removeChild(children[i]);
...
Tree.prototype.removeItem = function (id_or_node, remove_children) {
	...
	//remove all children
	if (child_nodes) {
		for (var i = 0; i < child_nodes.length; i++) this.root.removeChild(child_nodes[i]);
	}
...
Tree.prototype.updateItem = function (id, data) {
	...
	node.data = data;
	if (data.id) node.id = data.id; //this updateItemId ?
	if (data.content) {
...
Tree.prototype.unmarkAllAsSelected = function () {
	...
	if (selected_array) {
		for (var i = 0; i < selected_array.length; i++) selected_array[i].classList.remove("selected");
	}
	var semiselected = this.root.querySelectorAll(".ltreeitem.semiselected");
	for (var i = 0; i < semiselected.length; i++) semiselected[i].classList.remove("semiselected");
};
...
Tree.prototype._updateListBox = function (node, options, current_level) {
	...
	if ((options && options.collapsed) || current_level >= this.collapsed_depth) node.listbox.collapse();

	var child_elements = this.getChildren(node.dataset["item_id"]);
	if (!child_elements) return; //null

	if (child_elements.length) node.listbox.setEmpty(false);
	else node.listbox.setEmpty(true);
};
