...
class Dialog {
  ...
  static getDialog(id: string) {
    var element = document.getElementById(id) as any;
    if (!element) return null;
    return element.dialog;
  };

  static showAll() {
    var dialogs = document.body.querySelectorAll("litedialog") as any;
  ...
  _ctor(options: any) {
    ...
    this.minimized = true
    var panel = document.createElement("div") as any;
    ...
    if (options.buttons) {
      for (var i in options.buttons) this.addButton(options.buttons[i].name, options.buttons[i]);
    }
    ...
    if (close_button) close_button.addEventListener("click", this.close.bind(this));

    var maximize_button = panel.querySelector(".maximize-button");
    if (maximize_button) maximize_button.addEventListener("click", this.maximize.bind(this));

    var minimize_button = panel.querySelector(".minimize-button");
    if (minimize_button) minimize_button.addEventListener("click", this.minimize.bind(this));

    var hide_button = panel.querySelector(".hide-button");
    if (hide_button) hide_button.addEventListener("click", this.hide.bind(this));
    ...
    this.root.addEventListener("DOMNodeInsertedIntoDocument", () => {
      if (this.on_attached_to_DOM) this.on_attached_to_DOM();
      if (this.on_resize) this.on_resize();
    });
    this.root.addEventListener("DOMNodeRemovedFromDocument", () => {
      if (this.on_detached_from_DOM) this.on_detached_from_DOM();
    });
  ...
  setResizable() {
    ...
    footer.addEventListener("mousedown", (e: MouseEvent) => inner_mouse.call(this, e));
    corner.addEventListener("mousedown", (e: MouseEvent) => inner_mouse.call(this, e), true);
    ...
    function inner_mouse(this: any, e: MouseEvent) {
  ...
  detachWindow(on_complete?: any, on_close?: any) {
    ...
    if (header) title = header.textContent;

    var dialog_window = window.open(
      "",
      "",
    ...
    for (var i = 0; i < styles.length; i++) dialog_window.document.write(styles[i].outerHTML);
  ...
  dockTo(parent: any, dock_type: any) {
    if (!parent) return;
    var panel = this.root;
  ...
  minimize() {
    ...
    LiteGUI.bind(this, "closed", () => {
      LiteGUI.Dialog.minimized.splice(LiteGUI.Dialog.minimized.indexOf(this as any), 1);
 ...
 * @method show
 */
  show(reference_element?: any) {
    if (!this.root.parentNode) {
      if (!reference_element) LiteGUI.add(this);
      else {
